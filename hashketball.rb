...
def player_numbers(team)
  ...
  hash.each do |location, data |
    if data[:team_name] == team
      return data[:players].collect { |player| player[:number] }
    end
  end
end
...
def player_stats(name)
  hash = game_hash
  hash.each do |location, keys|
     keys[:players].each do |player|
       if player[:player_name] == name
         return player.delete_if { |k, v| [:player_name].include?(k)}
       end
     end
   end
end
